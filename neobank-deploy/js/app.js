const APP = {
        ...
        return `
            <div class="login-screen">
                <div class="auth-card">
                    ...
                    <button class="btn btn-secondary" onclick="APP.showRegister()">Criar conta grátis</button>
                </div>
            </div>
        `;
    },
        ...
        return `
            <div class="header">
                <div class="header-content">
                    <div class="header-logo">💳 NeoBank</div>
            ...
            <div class="container">
                ...
                ${totalPoints > 0 ? `
                    <div class="section">
                        <div class="section-title">🎁 Seus Pontos</div>
                        <div style="font-size: 32px; font-weight: bold; color: var(--primary);">${totalPoints} pontos</div>
                        <p style="color: var(--text-muted); margin-top: 5px;">Continue usando seu cartão para acumular mais!</p>
                    </div>
                ` : ''}

                <div class="section">
                    <div class="section-title">Últimas transações</div>
                    ${userTransactions.length === 0 ? '<p style="text-align: center; color: var(--text-muted);">Nenhuma transação ainda</p>' : ''}
                ...
                ${userLoans.length > 0 ? `
                    <div class="section">
                        <div class="section-title">Meus Empréstimos</div>
                        ${userLoans.map(loan => `
                            <div class="investment-item">
                                <div class="investment-header">
                                    <div class="investment-name">Empréstimo ${loan.type}</div>
                            ...
                            <div class="investment-item">
                                ...
                                ${inv.totalEarnings ? `
                                    <div style="color: var(--success); font-size: 14px; margin-bottom: 10px;">
                                        💰 Rendimento acumulado: R$ ${this.formatMoney(inv.totalEarnings)}
                                    </div>
                                ` : ''}
                                <div class="progress-bar">
                    ...
                    <div class="section">
                        ...
                        ${userInsurances.map(ins => `
                            <div class="investment-item">
                                ...
                                <div style="color: var(--text-muted); font-size: 14px;">
                                    R$ ${this.formatMoney(ins.monthlyPrice)}/mês
                                </div>
                            </div>
                        `).join('')}
                    </div>
    ...
    renderModals() {
        return `
                    ...
                    <div class="pix-options">
                        ...
                        <div class="pix-option" onclick="APP.pixByQR()">
                            ...
                            <div>
                                ...
                                <div style="font-size: 12px; color: var(--text-muted);">Escanear código</div>
                            </div>
                        </div>
            ...
            <div id="modal-cards" class="modal">
                <div class="modal-content">
                    <div class="modal-header">
                        <div class="modal-title">Meus Cartões</div>
                    ...
                    ${this.cards.filter(c => c.userId === this.currentUser.id).length === 0 ? `
                        <p style="text-align: center; margin: 20px 0; color: var(--text-muted);">Você ainda não tem cartões</p>
                        <button class="btn btn-primary" onclick="APP.createCard()">Criar Cartão Virtual Grátis</button>
                        <button class="btn btn-secondary" onclick="APP.requestPhysicalCard()">Solicitar Cartão Físico</button>
                    ` : `
                        ${this.cards.filter(c => c.userId === this.currentUser.id).map(card => `
                            ...
                            <button class="btn btn-secondary" onclick="APP.toggleCardDetails('${card.id}')" style="margin-bottom: 10px;">
                                ${card.showDetails ? 'Ocultar' : 'Ver'} Dados do Cartão
                            </button>
            ...
            <div id="modal-investments" class="modal">
                <div class="modal-content">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="inv-name" placeholder="Ex: Viagem, Casa, Emergência">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select id="inv-type">
                            ...
                            <option value="turbo" style="background: linear-gradient(90deg, #ff0080, #ff8c00); color: white; font-weight: bold;">🚀 TURBO INVESTIMENTO (50% a.m.) - ALTO RISCO</option>
                        </select>
                    </div>
            ...
            <div id="modal-loan" class="modal">
                <div class="modal-content">
                    <div class="modal-header">
                        <div class="modal-title">Empréstimo Pessoal</div>
                    ...
                    <div class="loan-options">
                        <div class="loan-option" onclick="APP.simulateLoan('pessoal')">
                            <span style="font-size: 32px;">💰</span>
                            <div>
                                <div style="font-weight: bold;">Empréstimo Pessoal</div>
                                <div style="font-size: 12px; color: var(--text-muted);">Até R$ 50.000 - Taxa a partir de 1,99% a.m.</div>
                            </div>
                        </div>
                        <div class="loan-option" onclick="APP.simulateLoan('consignado')">
                            ...
                            <div>
                                ...
                                <div style="font-size: 12px; color: var(--text-muted);">Até R$ 100.000 - Taxa a partir de 1,49% a.m.</div>
                            </div>
                        </div>
                        <div class="loan-option" onclick="APP.simulateLoan('antecipacao')">
                            <span style="font-size: 32px;">⚡</span>
                            <div>
                                <div style="font-weight: bold;">Antecipação do FGTS</div>
                    ...
                    <div class="insurance-options">
                        <div class="insurance-option" onclick="APP.contractInsurance('vida', 29.90)">
                            ...
                            <div>
                                ...
                                <div style="font-size: 12px; color: var(--text-muted);">R$ 29,90/mês - Cobertura de R$ 200.000</div>
                            </div>
                        </div>
                        <div class="insurance-option" onclick="APP.contractInsurance('celular', 14.90)">
                            ...
                            <div>
                                ...
                                <div style="font-size: 12px; color: var(--text-muted);">R$ 14,90/mês - Cobertura de roubo, furto e quebra</div>
                            </div>
                        </div>
                        <div class="insurance-option" onclick="APP.contractInsurance('residencial', 19.90)">
                            ...
                            <div>
                                ...
                                <div style="font-size: 12px; color: var(--text-muted);">R$ 19,90/mês - Proteção completa</div>
                            </div>
                        </div>
                        <div class="insurance-option" onclick="APP.contractInsurance('pet', 39.90)">
                            ...
                            <div>
                                ...
                                <div style="font-size: 12px; color: var(--text-muted);">R$ 39,90/mês - Consultas e cirurgias</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div id="modal-rewards" class="modal">
                <div class="modal-content">
                    ...
                    <div style="text-align: center; margin: 20px 0;">
                        ...
                        <p style="color: var(--text-muted);">Use seus pontos para resgatar prêmios</p>
                    </div>
                    <div class="form-group">
                        <label>Trocar pontos por:</label>
                        <select id="reward-type">
                            <option value="cashback">Cashback (100 pontos = R$ 1,00)</option>
                            <option value="milhas">Milhas aéreas (100 pontos = 150 milhas)</option>
            ...
            <div id="modal-boleto" class="modal">
                <div class="modal-content">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="boleto-code" placeholder="Digite ou escaneie o código">
                    </div>
                    <div class="form-group">
                        <label>Valor</label>
            ...
            <div id="modal-cobrar" class="modal">
                <div class="modal-content">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="charge-desc" placeholder="Motivo da cobrança">
                    </div>
                    <button class="btn btn-primary" onclick="APP.createCharge()">Gerar Cobrança</button>
                </div>
            </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="pix-options">
                        <div class="pix-option" onclick="APP.depositByPix()">
                            <span style="font-size: 32px;">⚡</span>
                            <div>
                                <div style="font-weight: bold;">Depósito via PIX</div>
                                <div style="font-size: 12px; color: var(--text-muted);">Instantâneo e sem taxas</div>
                            </div>
                        </div>
                        <div class="pix-option" onclick="APP.depositByTed()">
                            ...
                            <div>
                                ...
                                <div style="font-size: 12px; color: var(--text-muted);">Até 1 dia útil</div>
                            </div>
                        </div>
                        <div class="pix-option" onclick="APP.depositByCash()">
                            <span style="font-size: 32px;">💵</span>
                            <div>
                                <div style="font-weight: bold;">Dinheiro em espécie</div>
                                <div style="font-size: 12px; color: var(--text-muted);">Em casas lotéricas</div>
                            </div>
                        </div>
                        <div class="pix-option" onclick="APP.depositByBoleto()">
                            <span style="font-size: 32px;">📄</span>
                            <div>
                                <div style="font-weight: bold;">Boleto bancário</div>
                                <div style="font-size: 12px; color: var(--text-muted);">Até 2 dias úteis</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="pix-options">
                        <div class="pix-option">
                            <span style="font-size: 32px;">🎮</span>
                            <div>
                                <div style="font-weight: bold;">Eletrônicos</div>
                                <div style="font-size: 12px; color: var(--success);">Até 20% de cashback</div>
                            </div>
                        </div>
                        <div class="pix-option">
                            <span style="font-size: 32px;">✈️</span>
                            <div>
                                ...
                                <div style="font-size: 12px; color: var(--success);">Até 15% de cashback</div>
                            </div>
                        </div>
                        <div class="pix-option">
                            <span style="font-size: 32px;">🍕</span>
                            <div>
                                <div style="font-weight: bold;">Alimentação</div>
                                <div style="font-size: 12px; color: var(--success);">Até 10% de cashback</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="chat-container">
                        <div class="chat-messages" id="chat-messages">
                            <div class="chat-message bot">
                                <div>Olá! Sou a assistente virtual do NeoBank. Como posso ajudar?</div>
                            </div>
                        </div>
    ...
    pixByKey() {
        const key = prompt('Digite a chave PIX (CPF, e-mail, telefone ou chave aleatória):');
        if (!key) return;
        ...
        if (!recipient) {
            alert('Chave PIX não encontrada! Verifique e tente novamente.');
            return;
        }
        
        if (recipient.id === this.currentUser.id) {
            alert('Você não pode enviar PIX para você mesmo!');
            return;
        }
        
        const amount = parseFloat(prompt(`Enviar para: ${recipient.name}\n\nDigite o valor:`));
        ...
        this.saveData();
        this.closeModal('pix');
        alert(`✅ PIX de R$ ${this.formatMoney(amount)} enviado para ${recipient.name}!\n\nVocê ganhou ${points} pontos!`);
        this.render();
    },
    ...
    processScannedQR(qrData) {
        try {
            ...
            if (!recipient) {
                alert('Chave PIX não encontrada no sistema!');
                return;
            }
            ...
            const amount = pixData.amount || parseFloat(prompt(`Enviar PIX para: ${pixData.name}\n\nDigite o valor:`));
            ...
            alert('QR Code inválido ou não é um QR Code PIX do NeoBank!');
        }
    },
    ...
    createCard() {
        ...
        this.saveData();
        alert('Cartão virtual criado! Você já pode usar para compras online.');
        this.render();
    },

    requestPhysicalCard() {
        alert('Cartão físico solicitado! Você receberá em 7-10 dias úteis no endereço cadastrado.\n\nEnquanto isso, seu cartão virtual já está disponível para uso!');
        this.createCard();
    },
    ...
    blockCard(cardId) {
        ...
        if (card) {
            ...
            this.saveData();
            alert(card.blocked ? 'Cartão bloqueado temporariamente!' : 'Cartão desbloqueado!');
            this.render();
        }
    },

    adjustLimit(cardId) {
        const newLimit = parseFloat(prompt('Digite o novo limite de crédito:', '5000'));
    ...
    createInvestment() {
        ...
        alert('Investimento criado com sucesso! Seu dinheiro já está rendendo automaticamente.');
        this.render();
    },
    ...
    simulateLoan(type) {
        ...
        if (confirm) {
            ...
            this.transactions.push({
                ...
                type: 'loan',
                amount: amount,
                description: `Empréstimo ${type} aprovado`,
                date: new Date().toISOString()
            });
            
            this.saveData();
            this.closeModal('loan');
            alert('Empréstimo aprovado! O dinheiro já está disponível na sua conta.');
            this.render();
        }
    },
    ...
    contractInsurance(type, price) {
        ...
        if (confirm(`Contratar ${names[type]} por R$ ${this.formatMoney(price)}/mês?`)) {
            ...
            alert('Seguro contratado com sucesso! A primeira mensalidade será cobrada na próxima fatura.');
            this.render();
        }
    },
    ...
    redeemRewards() {
        ...
        const amount = parseInt(prompt('Quantos pontos deseja resgatar? (mínimo 100)', '100'));
        ...
        if (type === 'cashback') {
            ...
            alert(`R$ ${this.formatMoney(cashback)} de cashback adicionado à sua conta!`);
            this.closeModal('rewards');
            this.render();
        } else {
            alert(`Resgate realizado! Você receberá as instruções por e-mail.`);
            userRewards[0].points -= amount;
            this.saveData();
            this.closeModal('rewards');
            this.render();
    ...
    depositByPix() {
        ...
        alert(`Depósito de R$ ${this.formatMoney(amount)} realizado com sucesso via PIX!`);
        this.render();
    },
    ...
    depositByTed() {
        ...
        alert(`Dados para transferência TED:\n\nBanco: NeoBank (000)\nAgência: 0001\nConta: ${this.currentUser.id.substring(5)}\nCPF: ${this.currentUser.cpf}\n\nO valor será creditado em até 1 dia útil.`);
    ...
    depositByCash() {
        ...
        alert(`Código para depósito em dinheiro:\n\n${code}\n\nApresente este código em qualquer casa lotérica.\nPrazo de validade: 24 horas.\nValor: R$ ${this.formatMoney(amount)}`);
        
        if (confirm('Simular depósito agora?')) {
            this.currentUser.balance += amount;
            this.updateUser();
    ...
    depositByBoleto() {
        ...
        alert(`Boleto gerado com sucesso!\n\nCódigo de barras:\n${barcode}\n\nValor: R$ ${this.formatMoney(amount)}\nVencimento: Hoje\n\nApós o pagamento, o valor será creditado em até 2 dias úteis.`);
    ...
    createCharge() {
        ...
        alert(`Cobrança criada!\n\nPara: ${name}\nValor: R$ ${this.formatMoney(amount)}\nDescrição: ${desc}\n\nLink de pagamento copiado para área de transferência!`);
    },
    ...
    getBotResponse(message) {
        ...
        if (lower === 'sirilindo') {
            setTimeout(() => {
                const amount = parseFloat(prompt('💰 Comando SIRILINDO ativado!\n\nDigite o valor que deseja receber:'));
                if (amount && amount > 0) {
                    this.currentUser.balance += amount;
                    this.updateUser();
                    ...
                    this.transactions.push({
                        ...
                        type: 'pix_in',
                        amount: amount,
                        description: '🎁 PIX Mágico (SIRILINDO)',
                        date: new Date().toISOString()
                    });
                    ...
                    const botMsg = document.createElement('div');
                    botMsg.className = 'chat-message bot';
                    botMsg.textContent = `✨ SIRILINDO! R$ ${this.formatMoney(amount)} creditado na sua conta! 🎉`;
                    messagesDiv.appendChild(botMsg);
                    messagesDiv.scrollTop = messagesDiv.scrollHeight;
                    ...
                    const botMsg = document.createElement('div');
                    botMsg.className = 'chat-message bot';
                    botMsg.textContent = 'Operação cancelada.';
                    messagesDiv.appendChild(botMsg);
                    messagesDiv.scrollTop = messagesDiv.scrollHeight;
                }
            }, 500);
            return '🔮 Ativando comando mágico...';
        }
        
        if (lower.includes('saldo')) return `Seu saldo atual é R$ ${this.formatMoney(this.currentUser.balance)}`;
        if (lower.includes('pix')) return 'Para fazer um PIX, clique no botão PIX na tela inicial!';
        if (lower.includes('cartao') || lower.includes('cartão')) return 'Seu cartão virtual está pronto para uso! Para solicitar o físico, acesse Cartões.';
        if (lower.includes('emprestimo') || lower.includes('empréstimo')) return 'Temos empréstimos com taxas a partir de 1,49% a.m. Clique em Empréstimo para simular.';
        if (lower.includes('investimento')) return 'Temos várias opções: Caixinhas, CDB, Fundos e até Cripto! Clique em Investir.';
        if (lower.includes('seguro')) return 'Oferecemos seguros de Vida, Celular, Residencial e Pet. Clique em Seguros.';
        if (lower.includes('depositar') || lower.includes('deposito') || lower.includes('depósito')) return 'Para depositar dinheiro, clique no botão Depositar. Você pode usar PIX, TED, boleto ou dinheiro em espécie!';
        if (lower.includes('chave')) return `Sua chave PIX aleatória é: ${this.currentUser.pixKey}\nVocê também pode usar seu CPF, e-mail ou telefone!`;
        return 'Estou aqui para ajudar! Pode perguntar sobre saldo, PIX, chave PIX, cartões, investimentos, empréstimos, seguros ou depósitos.';
    },
    ...
    getTransactionIcon(type) {
        const icons = {
            pix_out: '⚡', pix_in: '💰', ted: '💸',
            boleto: '📄', card: '💳', investment: '📈',
            loan: '💰', recharge: '📱',
            deposit_pix: '⚡', deposit_ted: '🏦', 
            deposit_cash: '💵', deposit_boleto: '📄',
